...
class ProcessCampaignMessagesJob implements ShouldQueue
{
    ...
    private function markCampaignAsCompleted($campaignId)
    {
        ...
        // First, check if there are any pending or ongoing logs
        $hasPendingOrOngoing = CampaignLog::where('campaign_id', $campaignId)
            ->whereIn('status', ['pending', 'ongoing'])
            ->exists();

        if ($hasPendingOrOngoing) {
            // Still processing, don't mark as completed
            return;
        }

        // Check if there are any retryable failed logs
        ...
        if (!$failedLogsExists) {
            // No more pending/ongoing/retryable logs, mark the campaign as completed
