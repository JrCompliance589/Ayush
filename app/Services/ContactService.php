...
class ContactService
{
    ...
    public function store(object $request, $uuid = null){
        ...
        // Sync contact to sheet on update (delayed, fetches fresh data)
        if ($uuid !== null) {
            try {
                $syncCacheKey = "sheet_sync_pending_{$contact->organization_id}_{$contact->id}";
                if (!\Illuminate\Support\Facades\Cache::get($syncCacheKey)) {
                    \Illuminate\Support\Facades\Cache::put($syncCacheKey, true, 120);
                    \App\Jobs\SyncContactToSheetJob::dispatch($contact->organization_id, $contact->id)
                        ->delay(now()->addSeconds(30));
                }
            } catch (\Exception $e) {
                \Illuminate\Support\Facades\Log::warning("SheetSync dispatch failed: " . $e->getMessage());
            }
        }
