...
class SubscriptionPlanService
{
    ...
    public function store(Object $request)
    {
        ...
        // Add lifetime-specific limits if period is lifetime
        if ($request->input('period') === 'lifetime') {
            $metadata['daily_campaign_limit'] = $request->input('daily_campaign_limit', -1);
            $metadata['daily_contacts_limit'] = $request->input('daily_contacts_limit', -1);
    ...
    public function update(Object $request, $uuid)
    {
        ...
        if ($request->input('period') === 'lifetime') {
            ...
            // Remove lifetime-specific fields if switching away from lifetime
            unset($metadata['daily_campaign_limit']);
            unset($metadata['daily_contacts_limit']);
            unset($metadata['is_lifetime']);
        }
