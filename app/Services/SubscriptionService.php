...
class SubscriptionService
{
    ...
    public static function store($request, $organizationId, $planId, $userId)
    {
        ...
        if ($billingDetails['amountDue'] == 0) {
            ...
            Log::debug('inside subscriptions Service');
            Log::debug('billingDetails', ['billingDetails' => $billingDetails]);
            Log::debug('organizationId', ['organizationId' => $organizationId]);
            Log::debug('planId', ['planId' => $planId]);
            Log::debug('userId', ['userId' => $userId]);

            $netAmount = (float) str_replace(',', '', $billingDetails['netAmount']);
    ...
    public static function createBillingInvoice($billingDetails, $organizationId, $planId, $userId)
    {
        return DB::transaction(function () use ($billingDetails, $organizationId, $planId, $userId) {
            ...
            // Calculate validity based on plan period
