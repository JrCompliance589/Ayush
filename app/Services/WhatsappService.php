...
class WhatsappService
{
    ...
    public function sendMessage($contactUuId, $messageContent, $userId = NULL, $type="text", $buttons = [], $header = [], $footer = null, $buttonLabel = null)
    {
        ...
        // Sanitize message content: strip HTML tags but decode entities back for WhatsApp plain text
        $sanitizedMessage = html_entity_decode(clean($messageContent), ENT_QUOTES, 'UTF-8');
        ...
        if($type == "text"){
            ...
            $requestData['text']['body'] = $sanitizedMessage;
            ...
            if (!empty($header)) {
                // WhatsApp API restriction: Interactive LIST messages only support TEXT headers
                // Image, video, and document headers are NOT supported for list messages
                if ($type == "interactive list") {
                    // For list messages, only allow text headers
                    if (isset($header['type']) && $header['type'] === 'text' && isset($header['text'])) {
                        $requestData['interactive']['header'] = [
                            'type' => 'text',
                            'text' => $header['text']
                        ];
                    } elseif (isset($header['type']) && in_array($header['type'], ['image', 'video', 'document'])) {
                        // Log warning - media headers not supported for list messages
                        Log::warning("Interactive list messages do not support {$header['type']} headers. Header will be omitted.");
                        // Skip adding header for media types in list messages
                    } elseif (is_string($header)) {
                        // If header is passed as a simple string, treat it as text
                        $requestData['interactive']['header'] = [
                            'type' => 'text',
                            'text' => $header
                        ];
                    ...
                    // For button and CTA URL messages, media headers are allowed
                    $requestData['interactive']['header'] = $header;
                }
            }

            $requestData['interactive']['body']['text'] = $sanitizedMessage;

            if ($footer != null) {
                $requestData['interactive']['footer'] = [
                    'text' => html_entity_decode(clean($footer), ENT_QUOTES, 'UTF-8'),
                ];
            }
        }
        ...
        Log::info("This is request".json_encode($requestData));
        ...
        if($responseObject->success === true){
            $response['text']['body'] = $sanitizedMessage;
            $response['type'] = 'text';
    ...
    public function sendInteractiveMessage(
        ...
        // Sanitize message content: strip HTML tags but decode entities back for WhatsApp plain text
        $sanitizedMessage2 = html_entity_decode(clean($messageContent), ENT_QUOTES, 'UTF-8');
        ...
        if ($type === "text") {
            ...
            $requestData['text'] = [
                'preview_url' => true,
                'body' => $sanitizedMessage2,
            ];
        }
    
        // --- INTERACTIVE MESSAGES ---
        else if (in_array($type, ["interactive buttons", "interactive call to action url", "interactive list"])) {
            $requestData['type'] = 'interactive';
            ...
            // ✅ Fix: Proper structure for body text (no nesting)
            $requestData['interactive']['body'] = [
        ...
        Log::info("This is response".json_encode($responseObject));
        if($responseObject->success === true){
            $response['text']['body'] = $sanitizedMessage2;
            $response['type'] = 'text';
    ...
    private function buildTemplateChatMessage($templateContent, $contact){
        ...
        foreach($templateMetadatas as $templateMetadata){
            if($templateMetadata->type == 'HEADER'){
                ...
                } else if($templateMetadata->format == 'TEXT' && isset($templateMetadata->text)){
    ...
    public function sendMedia($contactUuId, $mediaType, $mediaFileName, $mediaFilePath, $mediaUrl, $location, $caption = NULL, $transcription = NULL)
    {
        ...
        Log::info("Media file path: {$mediaFilePath}, caption: {$caption}, transcription: {$transcription}");
        ...
        Log::info("This is media request".json_encode($requestData));
        Log::info("This is media headers".json_encode($headers));
        Log::info("This is media url".$url);
    ...
    public function createTemplate(Request $request)
    {
        ...
        Log::info("Creating template with URL: {$url}");
        $requestData = [
            "name" => $request->name,
        ...
        if($request->category == 'AUTHENTICATION'){
            ...
            if($request->body['text'] != null){
                ...
                if (!empty($request->body['example'])) {
                    ...
                    // Auto-detect variables like {{1}}, {{2}} in body text and generate placeholder examples
                    preg_match_all('/\{\{(\d+)\}\}/', $request->body['text'], $matches);
                    if (!empty($matches[1])) {
                        $exampleValues = [];
                        $maxVar = max(array_map('intval', $matches[1]));
                        for ($i = 1; $i <= $maxVar; $i++) {
                            $exampleValues[] = "sample_{$i}";
                        }
        ...
        if($request->category != 'AUTHENTICATION'){
            if ($request->has('buttons') && !empty($request->buttons)) {
                ...
                foreach ($request->buttons as $button) {
                    if ($button['type'] !== 'QUICK_REPLY') {
                        if ($button['type'] === 'URL') {
                            // Ensure url key exists before accessing it
                            if (!isset($button['url']) || empty($button['url'])) {
                                Log::warning('URL button missing url field', ['button' => $button]);
                                continue; // Skip this button if url is missing
                            }
                            $urlButton = [
                                'type' => $button['type'],
                                'text' => $button['text'] ?? '',
                            ...
                            // If URL contains variables like {{1}}, add example
                            if (preg_match('/\{\{\d+\}\}/', $button['url']) && !empty($button['example'])) {
                                $urlButton['example'] = is_array($button['example']) ? $button['example'] : [$button['example']];
                            }
                            $requestData['components'][count($requestData['components']) - 1]['buttons'][] = $urlButton;
                ...
                foreach ($request->buttons as $button) {
                    if ($button['type'] !== 'QUICK_REPLY') {
                        if ($button['type'] === 'URL') {
                            // Validate URL field
                            if (!isset($button['url']) || empty($button['url'])) {
                                Log::warning('Update: URL button missing url field', ['button' => $button]);
                                continue;
                            }
                            $requestData['components'][count($requestData['components']) - 1]['buttons'][] = [
                                'type' => $button['type'],
                            ...
                            $phoneNumber = ($button['country'] ?? '') . ($button['phone_number'] ?? '');
                            if (empty($phoneNumber)) {
                                Log::warning('Update: PHONE_NUMBER button missing phone number', ['button' => $button]);
                                continue;
                            }
                            $requestData['components'][count($requestData['components']) - 1]['buttons'][] = [
                                'type' => $button['type'],
    ...
    public function CarouselupdateTemplate(Request $request, $uuid)
{
    ...
    /** --------------------------------------------------------------
     *  🔥 FIX:
     *  - DO NOT remove example for HEADER (IMAGE) — required by Meta
     *  - Remove example from BUTTONS (not allowed)
     *  - Remove BODY example only if empty
     * --------------------------------------------------------------*/

    foreach ($requestData['components'] as &$component) {

        // BODY: remove example if body_text empty
        if (
            strtolower($component['type']) === 'body' &&
        ...
        // CAROUSEL handling
        if (strtolower($component['type']) === 'carousel') {

            foreach ($component['cards'] as &$card) {
                foreach ($card['components'] as &$cardComponent) {

                    // HEADER (IMAGE): KEEP example (required!)
                    if (
                        strtoupper($cardComponent['type']) === 'HEADER' &&
                        strtoupper($cardComponent['format']) === 'IMAGE'
                    ) {
                        // ensure example exists
                        if (!isset($cardComponent['example']) ||
                            !isset($cardComponent['example']['header_handle'])
                        ) {
                            // ❗ Meta requires header_handle example
                            $cardComponent['example'] = [
                                'header_handle' => [$request->header_handle ?? '']
                            ];
                        }

                        continue; // DO NOT remove example
                    }

                    // BUTTONS: remove all example fields
                    if (strtoupper($cardComponent['type']) === 'BUTTONS') {
                        foreach ($cardComponent['buttons'] as &$btn) {
                            if (isset($btn['example'])) {
                                unset($btn['example']);
                            }
                        }
                    }
    ...
    try {
        ...
        // Update DB
        $template->organization_id = session()->get('current_organization');
        $template->category = $template->status == 'APPROVED'
            ? $template->category
            : $request->category;
        ...
        $responseObject->success = false;
        $responseObject->data = (object)['error' => (object)[
