...
const contactFields = inject('contactFields', []) as Array<{id: number, name: string, type: string}>
...
const handleConnectable: HandleConnectableFunc = (node, connectedEdges) => {
  ...
  return connectedEdges.length <= 100
}
        ...
        <div class="flex items-center justify-between mt-2 mb-2">
            ...
            <div class="flex items-center space-x-3">
                ...
                <menubar-menu>
                  ...
                  <menubar-content class="max-h-64 overflow-y-auto">
                    ...
                    <menubar-item @click="addVariable('{street}')"> {street} </menubar-item>
                    <menubar-item @click="addVariable('{city}')"> {city} </menubar-item>
                    <menubar-item @click="addVariable('{state}')"> {state} </menubar-item>
                    <menubar-item @click="addVariable('{zip_code}')"> {zip_code} </menubar-item>
                    <menubar-item @click="addVariable('{country}')"> {country} </menubar-item>
                    <menubar-item @click="addVariable('{full_address}')"> {full_address} </menubar-item>
                    <menubar-item @click="addVariable('{organization_name}')"> {organization_name} </menubar-item>
                    <template v-if="dynamicVariables.length">
                      <div class="px-2 py-1 text-xs text-gray-500 font-semibold border-t mt-1 pt-1">{{ $t('Additional Details') }}</div>
                      <menubar-item v-for="variable in dynamicVariables" :key="variable.value" @click="addVariable(variable.value)"> {{ variable.label }} </menubar-item>
                    </template>
                  </menubar-content>
                </menubar-menu>
              </Menubar>
