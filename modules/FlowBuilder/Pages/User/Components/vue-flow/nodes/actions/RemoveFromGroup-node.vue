<template>
  <div class="remove-from-group-node">
    <Handle
      type="target"
    ...
    <div class="bg-white border-2 border-teal-200 rounded-lg p-4 shadow-sm w-[400px]" style="min-width: 400px;">
      ...
      <div class="flex items-center justify-between text-xs">
        ...
        <div class="flex items-center gap-1">
          <span v-if="config.wait_for_reply" class="bg-blue-100 text-blue-800 px-2 py-1 rounded-full text-xs">Wait Reply</span>
          <span 
    ...
    <div v-if="showEditModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white rounded-lg max-w-md w-full mx-4 max-h-[80vh] overflow-y-auto">
        <div class="px-6 py-4 border-b border-gray-200">
          <h3 class="text-lg font-semibold text-gray-900">Edit Remove from Group Action</h3>
        </div>
          ...
          <div class="space-y-4">
            ...
            <div>
              <div class="flex items-center">
                <FormCheckbox
                  v-model="editForm.config.wait_for_reply"
                  label="Wait for user reply before continuing"
                  class="h-4 w-4 text-teal-600 focus:ring-teal-500 border-gray-300 rounded"
                />
              </div>
              <p class="text-xs text-gray-500 mt-1 ml-6">If enabled, the flow will pause after this action and wait for the user's reply before proceeding to the next step.</p>
            </div>
...
const toggleEdit = () => {
  ...
  editForm.config = {
    ...
    wait_for_reply: config.value.wait_for_reply || false
  }
