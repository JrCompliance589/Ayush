...
// Get team permissions from shared Inertia props
const teamPermissions = computed(() => usePage().props.teamPermissions as string[] || []);
const teamRole = computed(() => usePage().props.teamRole as string);
...
const data = computed(() => ({
  navMain: allNavItems.filter(item => {
    const permission = permissionMap[item.title];
