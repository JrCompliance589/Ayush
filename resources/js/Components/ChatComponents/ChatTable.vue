...
export default {
    ...
    methods: {
        exportExcel() {
            ...
            // Build URL
            const url = `/chats/download-inbound?export=excel&days=${exportDays}`;
...
<template>
    ...
    <div class="px-4 pt-4 pb-3 border-b border-gray-100 flex-shrink-0">
        <div class="flex items-center justify-between mb-3">
            <div>
                <h2 class="text-xl font-bold text-gray-900">{{ $t('Chats') }}</h2>
                <p class="text-xs text-gray-500 mt-0.5">{{ rowCount }} {{ $t('conversations') }}</p>
            </div>
        </div>
        ...
        <div class="relative mb-2">
            ...
            <input @input="search" v-model="params.search" type="text"
                class="w-full pl-10 pr-10 py-2.5 bg-gray-50 border-0 rounded-xl text-sm focus:ring-2 focus:ring-[#ff5100] focus:bg-white transition-all"
                :placeholder="$t('Search name or number...')">
        ...
        <div class="mb-2">
            <button @click="showExport = !showExport" type="button"
                class="flex items-center gap-2 text-xs text-blue-600 hover:text-blue-800 transition-colors font-medium">
                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" class="transition-transform" :class="showExport ? 'rotate-90' : ''">
                    <path fill="currentColor" d="M8.59 16.59L13.17 12L8.59 7.41L10 6l6 6l-6 6z"/>
                </svg>
                {{ $t('Download') }} {{ $t('responded inbound chats') }}
            </button>
            <div v-if="showExport" class="mt-2 bg-blue-50 border border-blue-200 rounded-lg p-3">
                <form @submit.prevent="exportExcel" class="flex items-center gap-2 flex-wrap">
                    <select v-model="days"
                        class="px-2 py-1.5 border border-gray-300 rounded-lg text-xs focus:ring-2 focus:ring-[#ff5100] focus:border-[#ff5100]">
                    ...
                    <input v-if="days === 'custom'" v-model.number="customDays" type="number" min="1" max="30"
                        class="w-20 px-2 py-1.5 border border-gray-300 rounded-lg text-xs focus:ring-2 focus:ring-[#ff5100] focus:border-[#ff5100]"
                        :placeholder="$t('Days')" />
                    <button type="submit" :disabled="loading"
                        class="px-3 py-1.5 bg-[#ff5100] hover:bg-[#e64900] text-white rounded-lg text-xs font-medium transition-all disabled:opacity-50 disabled:cursor-not-allowed">
    ...
    <!-- Chat List - fills all remaining space -->
    <div ref="listRef" class="flex-1 overflow-y-auto min-h-0">
        ...
        <div class="flex items-center gap-3 px-4 py-3">
            ...
            <div class="flex-shrink-0">
                <img v-if="contact.avatar" class="w-11 h-11 rounded-full object-cover ring-2 ring-gray-100"
                    :src="contact.avatar">

                <div v-else-if="!!(contact.full_name?.trim()) && contact.full_name?.trim().length > 0"
                    class="w-11 h-11 rounded-full bg-gradient-to-br from-[#ff5100] to-orange-400 flex items-center justify-center text-white font-semibold text-base capitalize">
            ...
            <div class="flex-1 min-w-0">
                <div class="flex items-center justify-between mb-0.5">
                    <h3 class="font-semibold text-gray-900 truncate text-sm">{{ (contact.full_name?.trim() ||
                        contact.phone?.trim() || "N/A") }}</h3>
                    <span class="text-xs text-gray-400 ml-2 flex-shrink-0">{{ formatTime(contact?.last_chat?.created_at)
                    }}</span>
                </div>

                <div v-if="contact?.last_chat?.deleted_at === null" class="flex items-center justify-between">
                    <!-- Message Preview -->
                    <div class="text-xs text-gray-500 truncate flex-1 flex items-center gap-1.5">
                        <!-- Text -->
                    ...
                    <!-- Unread Badge -->
                    <span v-if="contact.unread_messages > 0"
                        class="ml-2 bg-[#ff5100] text-white rounded-full px-2 py-0.5 text-[10px] font-bold flex-shrink-0 min-w-[20px] text-center">
    ...
    <!-- Pagination - compact -->
    <div class="px-4 py-2 border-t border-gray-100 bg-white flex-shrink-0">
