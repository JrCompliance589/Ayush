...
const selectedTab = ref('all'); // all, pending, advance, repayment
...
const filteredExceptions = computed(() => {
    if (selectedTab.value === 'all') return exceptionsData.value.exceptions;
    return exceptionsData.value.exceptions.filter(e => e.exception_type === selectedTab.value);
});
...
const openResolveModal = (exception) => {
    selectedException.value = exception;
...
const openCancelModal = (exception) => {
    selectedException.value = exception;
...
const resolveException = async () => {
    try {
        ...
        //     `${apiBaseUrl}/payment-exceptions/${selectedException.value.id}/resolve`,
        //     {
        //         method: 'PATCH',
        ...
        if (result.success) {
            ...
            alert(result.message || 'Failed to resolve exception');
        }
    } catch (error) {
...
const cancelException = async () => {
    try {
        loading.value = true;
        // const response = await fetch(
        //     `${apiBaseUrl}/payment-exceptions/${selectedException.value.id}/cancel`,
        //     {
        //         method: 'PATCH',
        //         headers: { 'Content-Type': 'application/json' },
    ...
    headers: {
      'Content-Type': 'application/json',
        ...
        if (result.success) {
            ...
            alert(result.message || 'Failed to cancel exception');
        }
    } catch (error) {
        console.error('Error cancelling exception:', error);
        alert('Failed to cancel exception');
    } finally {
        loading.value = false;
    }
};
...
<template>
                ...
                <div v-if="isOpen"
                    ...
                    <div
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <h2 class="text-2xl font-bold text-white">Payment Exceptions</h2>
                                                            ...
                                                            <td class="px-6 py-4">
                                                                <div class="text-sm">
                                                                    ...
                                                                    <div class="text-green-600">Paid: ₹{{
                                                                        formatCurrency(exception.paid_amount) }}
                                                                    </div>
                                                            ...
                                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                                <div class="flex items-center gap-2">
                                                                    ...
                                                                    <button v-if="exception.status === 'pending'"
                                                                        @click="openResolveModal(exception)"
                                                                        class="text-green-600 hover:text-green-800 transition"
                                                                        title="Resolve">
                                                                        <CheckCircle class="w-5 h-5" />
                                                                    </button>
                                                                    <button v-if="exception.status === 'pending'"
                                                                        @click="openCancelModal(exception)"
        ...
        <div v-if="showExceptionDetail"
            ...
            <transition enter-active-class="transition-all duration-300 ease-out"
                ...
                <div v-if="showExceptionDetail && selectedException"
                            ...
                            <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                                ...
                                <div class="space-y-2">
                                    <div class="flex justify-between">
                                        ...
                                        <span class="text-sm font-medium text-gray-900">{{
                                            selectedException.customer_id }}</span>
                                    </div>
                                    <div class="flex justify-between">
                                    ...
                                    <div class="flex justify-between">
                                        ...
                                        <span class="text-sm font-medium text-gray-900">{{
                                            selectedException.proforma_no || 'N/A' }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div
                            ...
                            <div v-if="selectedException.tds_amount > 0"
                                ...
                                <div>
                                    ...
                                    <p class="text-sm font-semibold text-gray-900">{{ selectedException.tds_rate }}%
                                    </p>
                                </div>
                                <div>
                        ...
                        <div v-if="selectedException.status !== 'pending'"
                            ...
                            <div class="space-y-2">
                                ...
                                <div v-if="selectedException.resolved_by" class="flex justify-between">
                                    ...
                                    <span class="text-sm font-medium text-gray-900">{{
                                        selectedException.resolved_by }}</span>
                                </div>
                    ...
                    <!-- Footer Actions -->
                    <div v-if="selectedException.status === 'pending'"
                        class="border-t border-gray-200 p-6 bg-gray-50 flex gap-3">
                        <button @click="openResolveModal(selectedException)"
                            class="flex-1 px-6 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 transition font-medium flex items-center justify-center gap-2">
                            <CheckCircle class="w-5 h-5" />
                            Resolve Exception
                        </button>
                        <button @click="openCancelModal(selectedException)"
                            class="flex-1 px-6 py-3 bg-red-600 text-white rounded-lg hover:bg-red-700 transition font-medium flex items-center justify-center gap-2">
                            <XCircle class="w-5 h-5" />
                            Cancel Exception
                        </button>
                    </div>
    ...
    <transition enter-active-class="transition-opacity duration-300"
        ...
        <div v-if="showResolveModal"
            ...
            <div class="bg-white rounded-xl shadow-2xl w-full max-w-md">
                ...
                <div class="p-6">
                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700 mb-2">Resolved By (User ID)</label>
                        <input type="number" v-model="resolveForm.resolved_by" placeholder="Enter user ID"
                    ...
                    <div class="mb-6">
                        ...
                        <textarea v-model="resolveForm.resolution_notes" rows="4"
                            ...
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500"></textarea>
                    </div>
                    <div class="flex gap-3">
                        <button @click="resolveException" :disabled="loading"
                            class="flex-1 px-6 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 transition font-medium disabled:opacity-50">
                            {{ loading ? 'Resolving...' : 'Confirm' }}
                        </button>
                        <button @click="showResolveModal = false" :disabled="loading"
    ...
    <transition enter-active-class="transition-opacity duration-300"
        ...
        <div v-if="showCancelModal"
            ...
            <div class="bg-white rounded-xl shadow-2xl w-full max-w-md">
                <div class="p-6 border-b border-gray-200 bg-red-600">
                    <h3 class="text-xl font-bold text-white flex items-center gap-2">
                ...
                <div class="p-6">
                    <div class="mb-6">
                        ...
                        <textarea v-model="cancelForm.cancellation_reason" rows="4"
                            placeholder="Enter reason for cancellation..." required
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-red-500"></textarea>
                    </div>
                    <div class="flex gap-3">
                        <button @click="cancelException" :disabled="loading || !cancelForm.cancellation_reason.trim()"
                            class="flex-1 px-6 py-3 bg-red-600 text-white rounded-lg hover:bg-red-700 transition font-medium disabled:opacity-50">
                            {{ loading ? 'Cancelling...' : 'Confirm' }}
                        </button>
                        <button @click="showCancelModal = false" :disabled="loading"
                            class="flex-1 px-6 py-3 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition font-medium disabled:opacity-50">
                            Cancel
                        </button>
                    </div>
