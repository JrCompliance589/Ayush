...
<template>
  <AppLayout v-slot:default="slotProps">
    <div
      class="grid grid-cols-12 lg:h-[calc(100vh-65px)] bg-gradient-to-br from-gray-50 to-orange-50/30"
    >
      <!-- Chat List Sidebar -->
      <div
        class="col-span-12 lg:col-span-4 bg-white shadow-sm lg:shadow-xl flex flex-col h-[calc(100vh-65px)] overflow-hidden"
      >
        <ChatTable
          :rows="rows"
      ...
      <div
        v-if="contact"
        class="absolute inset-0 lg:inset-auto lg:relative col-span-12 lg:col-span-8 h-screen lg:h-[calc(100vh-65px)] overflow-y-auto"
      >
        <!-- Chat Header -->
        <ChatHeader
          :ticketingIsEnabled="ticketingIsEnabled"
        ...
        <div
          class="grid grid-rows-12 overflow-y-auto"
          :class="addon == 1 ? 'h-[calc(100%-130.1px)]' : 'h-[calc(100%-81px)]'"
        >
          <!-- Chat Messages Area -->
          <div
            v-if="!displayTemplate"
            class="flex-1 overflow-y-auto bg-[#efeae2] row-span-full chat-bg"
          ...
          >
            ...
            <Contact
              ...
              class="bg-white row-span-full pb-8"
              :fields="props.fields"
              :contact="contact"
          ...
          <!-- Chat Input Area -->
          <div
            v-if="!displayContactInfo && !formLoading && !displayTemplate"
          ...
          <div
            v-if="displayTemplate"
            class="flex-1 overflow-y-hidden bg-white row-span-full"
          >
            <CampaignForm
              v-if="displayTemplate"
              class="h-full"
      ...
      <div
        v-else
        class="hidden lg:block relative col-span-12 lg:col-span-8 h-screen lg:h-[calc(100vh-65px)] overflow-y-auto"
      >
        <div
          class="flex-1 flex items-center justify-center h-full p-8"
        >
...
onMounted(() => {
  ...
  echo
    .channel("chats.ch" + props.organizationId)
    .listen("NewChatEvent", (event) => {
      updateSidePanel(event.chat);
    });
