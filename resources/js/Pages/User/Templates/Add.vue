<template>
  <AppLayout>
    <div class="lg:h-[calc(100vh-65px)] bg-gradient-to-br from-slate-50 via-orange-50/30 to-slate-50 overflow-hidden">
      <!-- Header -->
      ...
      <!-- Main Content -->
      <div class="grid grid-cols-1 lg:grid-cols-2 lg:h-[calc(100vh-152px)]">
        <!-- Left Panel - Form -->
        ...
        <!-- Right Panel - Preview -->
        <div
          class="bg-gradient-to-br from-slate-100 via-orange-50/20 to-slate-100 p-4 flex items-center justify-center relative">
...
const isFormValid = computed(() => {
  ...
  // Validate buttons based on their type
  const hasInvalidButton = form.value.buttons.some((button) => {
    // All buttons must have type and text
    if (!button.type || button.type.trim() === "" || !button.text || button.text.trim() === "") {
      return true;
    }
    ...
    // PHONE_NUMBER buttons must have country and phone_number
    if (button.type === 'PHONE_NUMBER') {
      if (!button.country || button.country.trim() === '' || !button.phone_number || button.phone_number.trim() === '') {
        return true;
      }
    }
    
    // copy_code buttons must have example
    if (button.type === 'copy_code' && (!button.example || button.example.trim() === '')) {
      return true;
    }
